<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-sans">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0">
      <div>
        <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">🍯 Honig-Chargen</h1>
        <p class="text-gray-500 dark:text-gray-400 mt-1">Rechtssichere Loskennzeichnung, D.I.B. Gewährverschlüsse und Honigbuch-Verwendungsnachweis.</p>
      </div>
      ...
      <div class="flex items-center space-x-3">
        ...
        >
          ...
          <span>Honigbuch exportieren (CSV)</span>
        </button>
        <button 
    ...
    <div class="mb-8 bg-gradient-to-r from-amber-500/10 to-primary/10 border border-amber-500/20 rounded-3xl p-6 shadow-sm">
      <div class="flex items-center space-x-2.5 mb-3">
        ...
        <h3 class="text-lg font-bold text-gray-900 dark:text-white">KI-Chargen-Entwurf</h3>
      </div>
      <p class="text-xs text-gray-600 dark:text-gray-400 mb-4">
        Tippe oder diktiere deine Ernte- und Abfülldaten. Die KI extrahiert Sorte, Menge, D.I.B. Etiketten und Wassergehalt automatisch.
      </p>
      <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
        <textarea 
          v-model="aiDraftText" 
          placeholder="z.B. Gestern 42kg Sommertracht geerntet mit 16.5% Wassergehalt. Best-before date 2 Jahre. DIB Banderolen von Nummer 5001 bis 5084 verwendet und Rückstellprobe entnommen."
          rows="2"
        ...
        >
          <svg v-if="aiLoading" class="animate-spin h-4 w-4 text-white" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          <span v-else>Entwurf generieren</span>
        </button>
      </div>
    </div>
    ...
    <div v-if="showModal" class="bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-3xl shadow-md w-full max-w-3xl mx-auto p-6 mb-8 animate-scale">
      <div class="flex justify-between items-center mb-6 pb-4 border-b border-gray-100 dark:border-dark-border">
        <h3 class="text-xl font-bold text-gray-900 dark:text-white">
          {{ isEditMode ? '📝 Charge bearbeiten' : '🍯 Neue Honig-Charge erfassen' }}
        </h3>
      ...
      <form @submit.prevent="submitForm">
        <div class="space-y-6">
          ...
          <div>
            <h4 class="text-sm font-bold text-amber-500 uppercase tracking-wider mb-3">1. Allgemeine Informationen</h4>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div :class="selectedHoneyTypePreset === 'custom' ? 'md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4' : 'md:col-span-1'">
                <div :class="selectedHoneyTypePreset === 'custom' ? 'md:col-span-1' : 'w-full'">
                  <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Honigsorte *</label>
                  <select 
                  ...
                  >
                    <optgroup label="1. Spezifische botanische Herkunft">
                      <option v-for="t in DIB_HONEY_TYPES.specific" :key="t" :value="t">{{ t }}</option>
                    </optgroup>
                    <optgroup label="2. Mehr als eine botanische Herkunft">
                      <option v-for="t in DIB_HONEY_TYPES.double" :key="t" :value="t">{{ t }}</option>
                    </optgroup>
                    <optgroup label="3. Regionale / Topografische Herkunft">
                      <option v-for="t in DIB_HONEY_TYPES.regional" :key="t" :value="t">{{ t }}</option>
                    </optgroup>
                    <optgroup label="4. Unspezifische botanische Herkunft">
                      <option v-for="t in DIB_HONEY_TYPES.unspecific" :key="t" :value="t">{{ t }}</option>
                    </optgroup>
                    <optgroup label="5. Allgemeine Bezeichnungen">
                      <option v-for="t in DIB_HONEY_TYPES.general" :key="t" :value="t">{{ t }}</option>
                    </optgroup>
                    <option value="custom">Eigene Bezeichnung / Kombination...</option>
                  </select>
                </div>

                <div v-if="selectedHoneyTypePreset === 'custom'" class="md:col-span-2 animate-scale">
                  <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Eigene Bezeichnung / Kombination *</label>
                  <input 
                    v-model="form.honey_type" 
                    type="text" 
                    required
                    placeholder="z.B. Bayerischer Waldhonig oder Sommertracht mit Lindenhonig"
                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 dark:bg-dark-bg dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-primary text-sm"
                  />
                  <span class="text-[10px] text-gray-500 dark:text-gray-400 mt-1 block">
                    Hinweis: Bezeichnungen wie "Frühlingshonig" oder "Sommerhonig" sind laut Honigverordnung unzulässig. Bitte "Blüten" oder "Tracht" ergänzen.
                  </span>
                </div>
              </div>
              ...
              <div :class="selectedHoneyTypePreset === 'custom' ? 'md:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4' : 'contents'">
                <div>
                  <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Erntedatum *</label>
                  <input 
                ...
                <div>
                  <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Abfülldatum</label>
                  <input 
          ...
          <div>
            <h4 class="text-sm font-bold text-amber-500 uppercase tracking-wider mb-3">2. Mengen & Qualitätskontrolle</h4>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Erntemenge (kg) *</label>
                <input 
              ...
              <div>
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Wassergehalt (%)</label>
                <input 
                  ...
                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 dark:bg-dark-bg dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-primary text-sm"
                />
                <span v-if="form.water_content_percent > 18" class="text-[10px] text-red-500 font-bold block mt-1">⚠️ D.I.B.-Richtwert (&le;18%) überschritten!</span>
              </div>

              <div>
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Erwärmungstemp. (&deg;C)</label>
                <input 
          ...
          <div>
            <h4 class="text-sm font-bold text-amber-500 uppercase tracking-wider mb-3">3. Rechtskonformität & Loskennzeichnung</h4>
            <div class="bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-gray-800 rounded-2xl p-4 space-y-4">
              ...
              <div class="flex items-start space-x-3">
                ...
                <label for="is_exact_date" class="text-sm text-gray-700 dark:text-gray-300">
                  <span class="font-bold block">Taggenaues Mindesthaltbarkeitsdatum (MHD) verwenden</span>
                  <span class="text-xs text-gray-500 dark:text-gray-400 block mt-0.5">
                    Wenn das MHD taggenau (TT.MM.JJJJ) angegeben wird, ist laut Loskennzeichnungsverordnung (LKV) keine zusätzliche Losnummer erforderlich.
                  </span>
                </label>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                ...
                <div>
                  <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Mindesthaltbarkeitsdatum (MHD) *</label>
                  <input 
          ...
          <div>
            <h4 class="text-sm font-bold text-amber-500 uppercase tracking-wider mb-3">4. D.I.B. Gewährverschlüsse (Honigbuch)</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Gewährverschluss-Startnummer</label>
                <input 
                  v-model="form.dib_label_start" 
                  type="text" 
                  placeholder="z.B. 100401"
                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 dark:bg-dark-bg dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-primary text-sm font-mono"
                />
              </div>

              <div>
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Gewährverschluss-Endnummer</label>
                <input 
                  v-model="form.dib_label_end" 
                  type="text" 
                  placeholder="z.B. 100484"
                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 dark:bg-dark-bg dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-primary text-sm font-mono"
                />
              </div>
            </div>
            <p class="text-[10px] text-gray-500 dark:text-gray-400 mt-1.5 italic">
              Wird zur korrekten fortlaufenden Erfassung im gesetzlich vorgeschriebenen Honigbuch verwendet.
            </p>
          </div>

          <!-- Section 5: Reserve Sample -->
          <div>
            <h4 class="text-sm font-bold text-amber-500 uppercase tracking-wider mb-3">5. Qualitätssicherung: Rückstellprobe</h4>
            <div class="bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-gray-800 rounded-2xl p-4 space-y-4">
              <div class="flex items-start space-x-3">
                <input 
                ...
                <label for="reserve_sample_taken" class="text-sm text-gray-700 dark:text-gray-300">
                  <span class="font-bold block">Rückstellprobe entnommen (mind. 200g, kühl & dunkel gelagert)</span>
                </label>
              </div>

              <div v-if="form.reserve_sample_taken" class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 animate-scale">
                <div>
                  <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Entnahmedatum *</label>
                  <input 
          ...
          <!-- Section 6: Notes -->
          <div>
            <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Weitere Notizen / Bemerkungen</label>
            <textarea 
              v-model="form.notes" 
              placeholder="Besonderheiten bei Tracht, Cremigkeit, Rührung oder Lagerort..."
              rows="3"
              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 dark:bg-dark-bg dark:text-white rounded-xl focus:outline-none focus:ring-2 focus:ring-primary text-sm"
            ></textarea>
          </div>
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center py-20">
      ...
      <p class="text-gray-500 dark:text-gray-400 font-bold">Lade Honig-Chargen...</p>
    </div>
    ...
    <div v-else-if="batches.length === 0" class="glass rounded-3xl p-12 text-center max-w-lg mx-auto border border-dashed border-gray-300 dark:border-gray-700 mt-8">
      ...
      <h3 class="text-lg font-bold text-gray-800 dark:text-white mb-1">Keine Honig-Chargen erfasst</h3>
      <p class="text-gray-500 dark:text-gray-400 mb-6">Trage deine Honigernten und -abfüllungen ein, um die gesetzlichen Auflagen einfach zu erfüllen.</p>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800 text-left">
          <thead class="bg-gray-50 dark:bg-dark-bg text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
            <tr>
              <th scope="col" class="px-6 py-4">Los-Nr. / MHD</th>
              <th scope="col" class="px-6 py-4">Sorte</th>
              <th scope="col" class="px-6 py-4">Erntedatum</th>
              <th scope="col" class="px-6 py-4">Menge</th>
              <th scope="col" class="px-6 py-4">Qualität</th>
              <th scope="col" class="px-6 py-4">D.I.B. Gewährverschluss</th>
              <th scope="col" class="px-6 py-4">Rückstellprobe</th>
              <th scope="col" class="px-6 py-4 text-right">Aktionen</th>
            </tr>
          </thead>
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400 flex items-center space-x-1 mt-0.5">
                  ...
                  >
                    {{ b.is_exact_date ? 'Taggenau' : 'Monat/Jahr' }}
                  </span>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex flex-col space-y-1">
                  ...
                  <div v-if="b.water_content_percent !== null" class="flex items-center space-x-1">
                    <span class="text-xs text-gray-500 dark:text-gray-400">Wasser:</span>
                    <span 
                  ...
                  <div v-if="b.heating_temperature_celsius !== null" class="text-xs text-gray-500 dark:text-gray-400">
                    Erwärmung: <span class="font-mono text-gray-700 dark:text-gray-300">{{ b.heating_temperature_celsius }} °C</span>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-xs">
                <div v-if="b.dib_label_start || b.dib_label_end" class="font-mono text-gray-700 dark:text-gray-300">
                  ...
                  <div class="text-[10px] text-gray-500 dark:text-gray-400 font-bold mt-0.5">
                    ({{ getDIBCount(b.dib_label_start, b.dib_label_end) }} Banderolen)
                  </div>
                </div>
                <span v-else class="text-gray-400 dark:text-gray-600 italic">Keine D.I.B. Angaben</span>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <span v-else class="text-gray-400 dark:text-gray-600 italic">Nein</span>
              </td>
...
onMounted(async () => {
  ...
  if (pending) {
    try {
      ...
      showAlert('KI-Entwurf erfolgreich geladen!', 'success')
    } catch (e) {
...
async function fetchBatches() {
  ...
  try {
    ...
    showAlert('Fehler beim Laden der Honig-Chargen.', 'error')
  } finally {
    loading.value = false
  }
}
...
async function submitForm() {
  ...
  if (!form.is_exact_date && (!form.batch_number || !form.batch_number.trim())) {
    showAlert('Die Losnummer ist erforderlich, wenn das MHD nicht taggenau ist.', 'error')
    return
  }
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      showAlert('Honig-Charge erfolgreich aktualisiert!', 'success')
    } else {
      await axios.post('/api/honey-batches', payload, {
        params: { apiary_id: apiaryStore.activeApiaryId }
      })
      showAlert('Honig-Charge erfolgreich erfasst!', 'success')
    }
    ...
    showAlert(err.response?.data?.detail || 'Fehler beim Speichern der Honig-Charge.', 'error')
  }
}
...
async function deleteBatch(b) {
  ...
  try {
    ...
    showAlert(err.response?.data?.detail || 'Fehler beim Löschen der Honig-Charge.', 'error')
  }
}
...
async function generateAIDraft() {
  ...
  try {
    ...
    if (draft) {
      ...
      showModal.value = true
      aiDraftText.value = ''
      showAlert('KI-Entwurf erfolgreich auf das Formular angewendet!', 'success')
    } else {
      showAlert('Der Entwurf war leer oder ungültig.', 'error')
    }
  } catch (err) {
    console.error('AI Draft generation error:', err)
    showAlert('Fehler bei der Entwurfsgenerierung.', 'error')
  } finally {
...
async function exportCSV() {
  ...
  try {
    ...
    showAlert('Honigbuch erfolgreich exportiert!', 'success')
  } catch (err) {
